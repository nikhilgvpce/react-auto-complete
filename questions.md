...
Ans: In general, a Component in React accepts props from its Parent and use them to render output conditionally.

There is concept of `state` that a Component can maintain and is local or specific to that Component itself. Either the change in props or state makes the Component re-render and also when a prop or state in Parent Compoent changes then Parent Component re-renders and this makes all the Children components present in that Parent component to re-render irrespective whether the child components use the parent props or not.

Thus to prevent this issue of re-rendering of Children components we make use of PureComponents. Components that extend `PureComponent`, implement `shouldComponentUpdate` re-render only when the `shallow comparison` of `nextProps.someProp` with `this.props.someProp` and `nextState`  with `this.state.someState` is `false` meaning they are not same and one of them have changed and re-render is skipped when the shallow comparison is `true`.

A function or Component is said to be `Pure` if it doesn't interfer with or modify outside contents and for the same inputs it should return the same outputs when called any number of times.
...
***********************************************************************************************
  ...
  (b): `Include correct depencies in the dependency array in the hooks` provided by React. useEffect, useMemo, useCallback are the hooks help in prevent in the unnecessary component re-rendering. There is ceveat of using arrays, objects, functions as dependencies of these hooks, as they are recreated every tmime component renders and change their reference and these hooks make use of `Object.is()` comparision whether to render or not.
...
**********************************************************************************************
 ...
 Ans: Higher order components are those which can accept components as a prop. Below are the examples of the HOC pattern:
...
```
 ...
 as an input and passes in the updated props from parent and state from the store.

 (c): The `Styled-Components` library also follows HOC pattern where in the `ThemeProvider`, passes the `theme` prop to all its children inside it by creating a context which is achieved by following this HOC pattern.

(d): `withRouter` in `react-router` (version 5) is a higher component and takes in a component and passes the `location`, `history` data as props to it.
...
Ex: 
```
function getCartInfo(paymentInfo, cb) {
    const cartItems = {
        [
            itemId: 'd44fiui90a3',
            price: '$45'
...
`Promises` make use of `catch and fianlly block` to handle the exceptions in async code.
...
```
Similarly, `async` and `await` also use `try` and `catch` blocks to handle the async code.
...
setState is async because `React queues all the state update calls` and these states are commited to DOM at a time. If there are mutliple setState calls sequentially in each component then updating the DOM immediately with the each call of setState will be costly and will hinder the app performance. That is the reason React batches the setState calls and won't call them immediately.

***********************************************************************************************

### 9. List the steps needed to migrate a Class to Function Component.

Ans: To convert a Class component to a functional component we need to check for `life cycle methods` and state method used. Use respective hooks for functional components. Below is the way for converting life cycle methods to hooks:
...
so that it clears the clutter and dispatch the actions for state updates.
...
`
<Button outline={config.outline}>Click me!</Button>
```
